...
class CalendarController extends Controller
{
	...
	public function cal_editor($calendar_id = null, $calendar_size_id = null, $calendar_category_id=null,$month=null,$year=null){
		...
		if(isset(Auth::user()->email)){
			...
			$item_count = count($cartItems);
			/** end cart items **/
			return view('calendar.cal_custom_editor',compact('albums','photos','layout','background_image','demo_content','calendar_size','item_count','project_id','calendar_id','calendar_size_id','price','month','year','calendar_category_id','savedProj'));
		}else{
			return redirect('user/login');
		}
	}
	...
	function add_to_cart(Request $request){
		$project_id = $request->get('project_id');
		$_token = $request->get('_token'); 
		$exist = DB::table('carts')->whereRaw("user_id='". Auth::user()->id ."' AND project_id='".$project_id."' AND cart_type = 'Calendar'")->first();
	...
	public function collage_posters(){
		...
		return view('calendar.collage_poster',compact('col_pos_size','help_group_pages','resource_group_pages','corporate_group_pages','item_count'));
	}
	public function get_college_posters(Request $request){
		$whereRaw = '';
		...
		foreach($coll_pos_cal as $k => $value){
			...
			/** for image end **/
			$size_group = SizeGroup::where('sizegroup','=','Posters')->first();
			$size = Size::whereRaw("Size = '".$value['design_size']."' AND sizegroup = '".$size_group['id']."'")->first();
			$coll_pos_cal[$k]['calendar_size_id'] = $size['id'];
		}
		return view('calendar.ajax_college_poster',compact('coll_pos_cal'));exit;
	}
	public function colposview($calendar_id=null,$calendar_category_id=null){
		...
		$college_poster_sizes = CalendarDefaultSize::with('Size')->where('calendar_category_id',$calendar_category_id)->get();
	...
	public function poster_editor($calendar_id=null,$size_id=null,$calendar_category_id=null,$year=null){
		...
		if(isset(Auth::user()->email)){
			...
			$item_count = count($cartItems);
			/** end cart items **/
			return view('calendar.poster_editor',compact('albums','photos','layout','background_image','demo_content','calendar_size','item_count','project_id','calendar_id','size_id','price','year','calendar_category_id'));
		}else{
			return redirect('user/login');
		}
	}
