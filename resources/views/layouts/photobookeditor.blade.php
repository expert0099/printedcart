...
<html lang="en">
	...
	<body>
		...
		{!! Form::open(['enctype'=>'multipart/form-data','name'=>'photo_form','id'=>'photo_form']) !!}
			<div class="row">
				<div class="col-sm-12 form-group" style="text-align:left;">
					{!! Form::label('album_id', 'Album Name', ['class' => 'control-label']) !!}
					{!! Form::select('album_id', $album_list, ['class' => 'form-control', 'required'=>'true', 'id'=>'album_id']) !!}
				</div>
				<div class="col-sm-12 form-group" style="text-align:left;">
				...
				<div class="col-sm-12 form-group" style="text-align:left;">
					...
					{!! Form::file('images[]', ['class' => 'form-control', 'multiple'=>'true', 'required'=>'true','id'=>'images']) !!}
				</div>
			</div>
			{!! Form::submit('Upload', ['class' => 'btn btn-primary', 'id'=>'photo_button']) !!}
		{!! Form::close() !!}
		</div>
		<!-- end photo dialog -->
		...
		{!! Form::open(['method' => 'POST', 'url' => ['photobooks/add_new_album_ajax'],'id'=>'album_form']) !!}
			<div class="row">
...
$(function(){
			...
			$.ajax({
				...
				success : function(data){
					...
					if(data=='yes'){
						...
						$.ajax({
							url : base_path + 'photobooks/add_new_album_ajax',							
							type : 'post',
							data : {album_name:album_name},
							beforeSend: function(){
								//$('#img_loader').css('display','block');
	...
	/* $("#photo_button").click(function(){
		...
		alert(album_id);
		if(images == ''){
			var error_msg = '<p class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-label="Close" style="cursor:pointer;"><span aria-hidden="true">×</span></button>Oops! Please browse image.</p>';
			$('#photo_form .row').before(error_msg);
		}else{
			var frmData = new FormData($('#photo_form')[0]); 
			alert(frmData);
			$.ajaxSetup({ 
				headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') } 
			});
	...
	$("#photo_form").submit(function(){
		...
		$.ajax({
            url : base_path + 'photobooks/upload_new_images_ajax',            
            type : 'POST',
            data : frmData,
            processData: false,
            contentType: false,
			...
			success : function(data){
				...
				if(data=='ok'){
					...
					var errors_massage = [];
					//$(data.success).each(function(i, items) {
					var success_msg = '<p class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-label="Close" style="cursor:pointer;"><span aria-hidden="true">×</span></button>'+data.success+'</p>';
					errors_massage.push(success_msg);
					//});
					var m_error = data.err_msg;
					...
					$(data.error).each(function(i, item) {
						error_info += '<p class="alert alert-danger">'+item+'</p>';
					});
					error_info += "</div>";
