			...
			<div class="row">
				...
				<div id="transition-timer-carousel" class="carousel slide transition-timer-carousel {{$addClass}}" data-ride="carousel" data-interval="false">
					...
					<div class="carousel-inner vertical">
						...
						<!--<input type="hidden" name="target_crop_id" id="target_crop_id"/>-->
					</div>
					<!-- Controls -->
				</div>
			</div>
...
$(document).ready(function(){
	...
	/* $(".bg-img-inner").on('click',function(){
		alert('else');
	}); */
});
</script>
...
function drop(ev){
	...
	if(ev.target.id == ""){
		...
		$('#'+tId).append(parentdv);
		//ev.target.parentNode.parentNode.append(parentdv);
...
//alert(tId);
				...
				var subparent = document.createElement("div");
				subparent.className = "bg-img-inner";
				subparent.onclick = function() { crop(src2,tId); };
