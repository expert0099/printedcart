...
<script>
$(document).ready(function(){
	//var book_size = "<?php echo $book_size;?>";
	...
	/*if(book_size == '11x8'){
			$('#image-gallery').attr('height','331px !important');
		}*/
});
</script>
<script>
...
$(document).ready(function(){
	...
	var imageGallerySlider = $('#image-gallery').lightSlider({
		gallery:true,
		item:2,
		thumbItem:10,
		slideMargin: 0,
	...
	$('#addmore_page, #addmore_page2').click(function(){
		...
		$.ajax({
            ...
            success : function(data){
				var rand = Math.floor((Math.random() * 1000) + 900);
				var page_data = '<li id="'+rand+'" class="lslide" style="width: 480px; margin-right: 0px;">'+data+'</li>';
				$("#image-gallery li.active").after(page_data);
				//layoutAttrAfterAppend();
				swal("Done", "Page Added Successfully!!", "success");
				imageGallerySlider.refresh();
            }
        });
		//undoRedo.saveState();
		return false;
	...
	$('#save_project').click(function(){
		...
		if(project_name==''){
			//toastr.error("Sorry! Please enter your photo-book name!!");
			swal("Oops!", "Please enter your photo-book name...!", "error");
			$('#project_name').focus();
			return false;
		}else{
			var optionHtml = [];
			...
			$.ajax({
				...
				success : function(data){
					if(data=='saved'){
						...
						swal("Done", "Project save successfully!!", "success")
						.then((value) => {
	...
	$('#save_pages').click(function(){
		...
		if(project_id == null){
			//toastr.error("Please add project first from project nav menu!!");
			swal("Oops!", "Please add project first, from project nav menu...!", "error");
		}else{
			var counter = 0;
	...
	$('#preview').click(function(){
		...
		$.ajax({
			...
			beforeSend: function(){
				//toastr.success("Please wait...! Preview Loading Data!!");
