...
<script>
$(document).ready(function() {
	var imageSliderGallery = $('#image-gallery').lightSlider({
		gallery:true,
		item:2,
		thumbItem:10,
		slideMargin: 0,
			...
			$.ajax({
				...
				success : function(data){
					if(data=='saved'){
						...
						swal("Done", "Project Saved Successfully!!", "success");
					}
				}
			});
			return false;
		...
		$.ajax({
            ...
            success : function(data){
				if(data.status == 'added'){
					...
					swal("Done", "Project added successfully into cart!!", "success");
				}else{
					//toastr.error("Sorry! Already added into cart!!");
					swal("Oops!", "Aleady added into cart...!", "error");
				}
            }
        });
		return false;
